...
static bool RequestMapMatchEntry(const PLDHashEntryHdr* hdr, const void* key) {
  const RequestHashEntry* entry = static_cast<const RequestHashEntry*>(hdr);
  return entry->r == key;
}

static void RequestMapInitEntry(PLDHashEntryHdr* hdr, const void* key) {
  RequestHashEntry* entry = static_cast<RequestHashEntry*>(hdr);
  entry->r = (void*)key;
}

static const PLDHashTableOps gMapOps = {
    PLDHashTable::HashVoidPtrKeyStub, RequestMapMatchEntry,
...
// static
already_AddRefed<nsISupports> nsSecureBrowserUIImpl::ExtractSecurityInfo(
    nsIRequest* aRequest) {
  nsCOMPtr<nsISupports> retval;
  nsCOMPtr<nsIChannel> channel(do_QueryInterface(aRequest));
  if (channel) channel->GetSecurityInfo(getter_AddRefs(retval));

  if (!retval) {
    nsCOMPtr<nsISecurityInfoProvider> provider(do_QueryInterface(aRequest));
    if (provider) provider->GetSecurityInfo(getter_AddRefs(retval));
  }

  return retval.forget();
}

nsresult nsSecureBrowserUIImpl::MapInternalToExternalState(uint32_t* aState,
                                                           lockIconState lock,
                                                           bool ev) {
  ...
  // For content docShell's, the mixed content security state is set on the root
  // docShell.
  ...
  if (docShell->GetHasMixedActiveContentLoaded() &&
    ...
    *aState =
        tempState | nsIWebProgressListener::STATE_LOADED_MIXED_ACTIVE_CONTENT;
...
static uint32_t GetSecurityStateFromSecurityInfoAndRequest(
    nsISupports* info, nsIRequest* request) {
  nsresult res;
  ...
  MOZ_LOG(gSecureDocLog, LogLevel::Debug,
          ("SecureUI: GetSecurityState: - Returning %d\n", securityState));
...
void nsSecureBrowserUIImpl::EvaluateAndUpdateSecurityState(nsIRequest* aRequest,
                                                           nsISupports* info,
...
void nsSecureBrowserUIImpl::UpdateSubrequestMembers(nsISupports* securityInfo,
                                                    nsIRequest* request) {
  // For wyciwyg channels in subdocuments we only update our
  // subrequest state members.
  uint32_t reqState =
      GetSecurityStateFromSecurityInfoAndRequest(securityInfo, request);
...
nsSecureBrowserUIImpl::OnStateChange(nsIWebProgress* aWebProgress,
  ...
  /*
    ...
    with those redirects, the old page contents and their requests will come to
    STOP completely, before any progress from new refreshed page content is
    ...
    If the toplevel documents gets replaced, i.e. redirected with 302, we do not
    care for the security state of the initial transaction, which has now been
    redirected, we only care for the new page load.

    For the implementation of the security UI, we make an assumption, that is
    hopefully true.

    Imagine, the received page that was delivered with the 302 redirection
    answer, also delivered html content.

    What happens if the parser starts to analyze the content and tries to load
    contained sub objects?

    In that case we would see start and stop requests for subdocuments, some for
    the previous document, some for the new target document. And only those for
    the new toplevel document may be taken into consideration, when deciding
    about the security state of the next toplevel document.

    Because security state is being looked at, when loading stops for
    (sub)documents, this could cause real confusion, because we have to decide,
    whether an incoming progress belongs to the new toplevel page, or the
    previous, already redirected page.

    Can we simplify here?

    If a redirect at the http protocol level is seen, can we safely assume, its
    html content will not be parsed, anylzed, and no embedded objects will get
    loaded (css, js, images), because the redirect is already happening?

    If we can assume that, this really simplify things. Because we will never
    see notification for sub requests that need to get ignored.

    I would like to make this assumption for now, but please let me (kaie) know
    if I'm wrong.

    Excurse:
      If my assumption is wrong, then we would require more tracking
    information. We need to keep lists of all pointers to request object that
    had been seen since the last toplevel start event. If the start for a
    redirected page is seen, the list of releveant object must be cleared, and
    only progress for requests which start after it must be analyzed. All other
    events must be ignored, as they belong to now irrelevant previous top level
    documents.
    ...
    kaie thinks:
      Only if the toplevel frame is secure, we should try to display secure lock
    icons. If some of the inner contents are insecure, we display mixed mode.

      But if the top level frame is not secure, why indicate a mixed lock icon
    at all? I think we should always display an open lock icon, if the top level
    frameset is insecure.

      That's the way Netscape Communicator behaves, and I think we should do the
    same.

      The user will not know which parts are secure and which are not,
      and any certificate information, displayed in the tooltip or in the "page
    info" will only be relevant for some subframe(s), and the user will not know
    which ones, so we shouldn't display it as a general attribute of the
    displayed page.

    Why are frames evil?

    Because the progress for the toplevel frame document is not easily
    distinguishable from subframes. The same STATE bits are reported.

    While at first sight, when a new page load happens,
    the toplevel frameset document has also the STATE_IS_NETWORK bit in it.
    But this can't really be used. Because in case that document causes a http
    302 redirect, the real top level frameset will no longer have that bit.

    But we need some way to distinguish top level frames from inner frames.

    I saw that the web progress we get delivered has a reference to the toplevel
    DOM window.

    I suggest, we look at all incoming requests.
    If a request is NOT for the toplevel DOM window, we will always treat it as
    a subdocument request, regardless of whether the load flags indicate a top
    level document.
  */
  ...
  if (httpChannel) {
    uint32_t response;
    isNoContentResponse =
        NS_SUCCEEDED(httpChannel->GetResponseStatus(&response)) &&
        (response == 204 || response == 205);
  }
  const bool isToplevelProgress =
      (windowForProgress.get() == window.get()) && !isNoContentResponse;
  ...
  if (aProgressStateFlags & STATE_START &&
      aProgressStateFlags & STATE_IS_REQUEST && isToplevelProgress &&
  ...
  if (uri && mIOService) {
    bool hasFlag;
    nsresult rv = mIOService->URIChainHasFlags(
        uri, nsIProtocolHandler::URI_IS_LOCAL_RESOURCE, &hasFlag);
...
#if defined(DEBUG)
  if (aProgressStateFlags & STATE_STOP && channel) {
    MOZ_LOG(
        gSecureDocLog, LogLevel::Debug,
  ...
  if (aProgressStateFlags & STATE_STOP &&
    ...
    if (!requestHasTransferedData) {
      ...
      requestHasTransferedData =
          ...
          (NS_FAILED(
               securityInfoProvider->GetHasTransferredData(&hasTransferred)) ||
           hasTransferred);
    }
  }
  ...
  if (aProgressStateFlags & STATE_START &&
    ...
    if (allowSecurityStateChange && !inProgress) {
      ...
      if (prevContentSecurity) {
        ...
        MOZ_LOG(gSecureDocLog, LogLevel::Debug,
                ("SecureUI:%p: Saving subs in START to %p as %d,%d\n", this,
                 prevContentSecurity.get(), saveSubBroken, saveSubNo));
      }

      bool retrieveAssociatedState = false;

      if (securityInfo && (aProgressStateFlags &
                           nsIWebProgressListener::STATE_RESTORING) != 0) {
      ...
      if (retrieveAssociatedState) {
        ...
        nsCOMPtr<nsIAssociatedContentSecurity> newContentSecurity(
        ...
        if (newContentSecurity) {
          ...
          MOZ_LOG(gSecureDocLog, LogLevel::Debug,
                  ("SecureUI:%p: Restoring subs in START from %p to %d,%d\n",
        ...
        // load with all zeros and break mixed content state on back and
        // forward.
  ...
  if (aProgressStateFlags & STATE_STOP &&
    ...
    if (mDocumentRequestsInProgress <= 0) {
      ...
      // Unfortunately on application start, see some stops without having seen
      // any starts...
      return NS_OK;
    }
    ...
    if (mRestoreSubrequests && !inProgress) {
      ...
      // been rendered but has been retargeted in some other way then by
      // external app handler.  Restore mSubRequests* members to what the
      // current security state info holds (it was reset to all zero in
      // OnStateChange(START) before).
      ...
      if (currentContentSecurity) {
        ...
        MOZ_LOG(gSecureDocLog, LogLevel::Debug,
                ("SecureUI:%p: Restoring subs in STOP from %p to %d,%d\n", this,
                 currentContentSecurity.get(), subBroken, subNo));
      }
  ...
  if (aProgressStateFlags & STATE_STOP &&
      aProgressStateFlags & STATE_IS_REQUEST) {
    if (!isSubDocumentRelevant) return NS_OK;

    // if we arrive here, LOAD_DOCUMENT_URI is not set

    // We only care for the security state of sub requests which have actually
    // transfered data.
    ...
    if (allowSecurityStateChange && requestHasTransferedData) {
      ...
      // but the security state of the new top level document might not yet been
      // known.
      //
      // At this point, we are learning about the security state of a
      // sub-document. We must not update the security state based on the sub
      // content, if the new top level state is not yet known.
      //
...
void nsSecureBrowserUIImpl::ObtainEventSink(
    nsIChannel* channel, nsCOMPtr<nsISecurityEventSink>& sink) {
  if (!sink) NS_QueryNotificationCallbacks(channel, sink);
}

void nsSecureBrowserUIImpl::UpdateSecurityState(nsIRequest* aRequest,
                                                bool withNewLocation,
  ...
  mCertUserOverridden = mNewToplevelSecurityState & STATE_CERT_USER_OVERRIDDEN;
...
void nsSecureBrowserUIImpl::TellTheWorld(nsIRequest* aRequest) {
...
nsSecureBrowserUIImpl::OnLocationChange(nsIWebProgress* aWebProgress,
  ...
  if (aFlags & LOCATION_CHANGE_SAME_DOCUMENT) return NS_OK;
...
nsresult nsSecureBrowserUIImpl::OnSecurityChange(nsIWebProgress* aWebProgress,
                                                 nsIRequest* aRequest,
                                                 uint32_t state) {
  MOZ_ASSERT(NS_IsMainThread());
#if defined(DEBUG)
  ...
  switch (mNotifiedSecurityState) {
    ...
    default:
      MOZ_FALLTHROUGH_ASSERT(
          "if this is reached you must add more entries to the switch");
    case lis_no_security:
      *_result = nullptr;
      return NS_OK;
  }
