...
nsDocShell::LoadURI(nsIURI* aURI, nsIDocShellLoadInfo* aLoadInfo,
                    uint32_t aLoadFlags, bool aFirstParty) {
  ...
  MOZ_LOG(gDocShellLeakLog, LogLevel::Debug,
          ("nsDocShell[%p]: loading %s with flags 0x%08x", this,
           aURI->GetSpecOrDefault().get(), aLoadFlags));

  if (!shEntry && !LOAD_TYPE_HAS_FLAGS(loadType, LOAD_FLAGS_REPLACE_HISTORY)) {
    ...
    if (parentDS && parentDS != static_cast<nsIDocShell*>(this)) {
      ...
      if (!mCurrentURI ||
          (NS_IsAboutBlank(mCurrentURI) && !currentChildEntry)) {
  ...
  // a principal from the referrer, since the referrer URI reflects the web
  // origin that triggered the load. If there is no referrer URI, we fall back
  // to using the SystemPrincipal. It's safe to assume that no provided
  // triggeringPrincipal and no referrer simulate a load that was triggered by
  // the system. It's important to note that this block of code needs to appear
  // *after* the block where we munge the principalToInherit, because otherwise
  // we would never enter code blocks checking if the principalToInherit is null
  // and we will end up with a wrong inheritPrincipal flag.
  if (!triggeringPrincipal) {
    if (referrer) {
      nsresult rv = CreatePrincipalFromReferrer(
          referrer, getter_AddRefs(triggeringPrincipal));
      NS_ENSURE_SUCCESS(rv, rv);
    } else {
  ...
  return InternalLoad(aURI, originalURI, resultPrincipalURI,
                      keepResultPrincipalURIIfSet, loadReplace,
                      isFromProcessingFrameAttributes, referrer, referrerPolicy,
                      triggeringPrincipal, principalToInherit, flags, target,
                      nullptr,       // No type hint
                      VoidString(),  // No forced download
                      postStream,
                      -1,  // XXXbaku
                      headersStream, loadType,
                      nullptr,  // No SHEntry
                      aFirstParty, srcdoc, sourceDocShell, baseURI,
...
nsDocShell::LoadStream(nsIInputStream* aStream, nsIURI* aURI,
  ...
  nsresult rv = NS_NewInputStreamChannel(
      getter_AddRefs(channel), uri, stream.forget(), triggeringPrincipal,
      nsILoadInfo::SEC_ALLOW_CROSS_ORIGIN_DATA_IS_NULL,
      nsIContentPolicy::TYPE_OTHER, aContentType, aContentCharset);
  NS_ENSURE_SUCCESS(rv, NS_ERROR_FAILURE);

  nsCOMPtr<nsIURILoader> uriLoader(do_GetService(NS_URI_LOADER_CONTRACTID));
  NS_ENSURE_TRUE(uriLoader, NS_ERROR_FAILURE);

  NS_ENSURE_SUCCESS(DoChannelLoad(channel, uriLoader, false), NS_ERROR_FAILURE);
  return NS_OK;
}
...
nsresult nsDocShell::GetEldestPresContext(nsPresContext** aPresContext) {
...
nsDocShell::SetUsePrivateBrowsing(bool aUsePrivateBrowsing) {
  nsContentUtils::ReportToConsoleNonLocalized(
      NS_LITERAL_STRING("Only internal code is allowed to set the "
                        "usePrivateBrowsing attribute"),
      nsIScriptError::warningFlag, NS_LITERAL_CSTRING("Internal API Used"),
...
ScreenOrientationInternal nsDocShell::OrientationLock() {
  return mOrientationLock;
}

void nsDocShell::SetOrientationLock(
...
nsresult nsDocShell::HistoryTransactionRemoved(int32_t aIndex) {
  // These indices are used for fastback cache eviction, to determine
  // which session history entries are candidates for content viewer
  // eviction.  We need to adjust by the number of entries that we
  // just purged from history, so that we look at the right session history
...
nsresult nsDocShell::SetDocLoaderParent(nsDocLoader* aParent) {
  ...
  if (parentAsDocShell) {
    ...
    if (NS_SUCCEEDED(
            parentAsDocShell->GetTouchEventsOverride(&touchEventsOverride))) {
...
static void PrintDocTree(nsIDocShellTreeItem* aParentNode, int aLevel) {
  ...
  printf("DS %p  Ty %s  Doc %p DW %p EM %p CN %p\n",
         ...
         type == nsIDocShellTreeItem::typeChrome ? "Chr" : "Con", (void*)doc,
         (void*)domwin.get(), (void*)presContext->EventStateManager(),
         (void*)rootElement);
...
static void PrintDocTree(nsIDocShellTreeItem* aParentNode) {
...
NS_IMETHODIMP
nsDocShell::LoadURI(const char16_t* aURI, uint32_t aLoadFlags,
                    nsIURI* aReferringURI, nsIInputStream* aPostStream,
                    nsIInputStream* aHeaderStream,
                    nsIPrincipal* aTriggeringPrincipal) {
  return LoadURIWithOptions(aURI, aLoadFlags, aReferringURI,
                            mozilla::net::RP_Unset, aPostStream, aHeaderStream,
                            nullptr, aTriggeringPrincipal);
}

NS_IMETHODIMP
nsDocShell::LoadURIWithOptions(const char16_t* aURI, uint32_t aLoadFlags,
...
#define SAFE_ESCAPE(output, input, params)             \
  ...
  return InternalLoad(
      errorPageURI, nullptr, Nothing(), false, false, false, nullptr,
      mozilla::net::RP_Unset, nsContentUtils::GetSystemPrincipal(), nullptr,
      INTERNAL_LOAD_FLAGS_NONE, EmptyString(), nullptr, VoidString(), nullptr,
      -1, nullptr, LOAD_ERROR_PAGE, nullptr, true, VoidString(), this, nullptr,
      nullptr, nullptr);
}
...
nsDocShell::ForceRefreshURI(nsIURI* aURI, nsIPrincipal* aPrincipal,
   ...
   */
  LoadURI(aURI, loadInfo,
          nsIWebNavigation::LOAD_FLAGS_DISALLOW_INHERIT_PRINCIPAL, true);
    ...
    if (sURIFixup) {
      ...
      if (newURI) {
        ...
        if (!sameURI) {
          ...
          nsCOMPtr<nsIPrincipal> triggeringPrincipal =
              loadInfo ? loadInfo->TriggeringPrincipal()
...
class InternalLoadEvent : public Runnable {
 public:
  InternalLoadEvent(nsDocShell* aDocShell, nsIURI* aURI, nsIURI* aOriginalURI,
                    ...
                    int64_t aPostDataLength, nsIInputStream* aHeadersData,
                    uint32_t aLoadType, nsISHEntry* aSHEntry, bool aFirstParty,
  ...
  Run() override {
    return mDocShell->InternalLoad(
        ...
        EmptyString(), mTypeHint.IsVoid() ? nullptr : mTypeHint.get(),
        VoidString(), mPostData, mPostDataLength, mHeadersData, mLoadType,
        mSHEntry, mFirstParty, mSrcdoc, mSourceDocShell, mBaseURI, nullptr,
        nullptr);
  }
...
nsresult nsDocShell::CreatePrincipalFromReferrer(nsIURI* aReferrer,
...
NS_IMETHODIMP
nsDocShell::InternalLoad(nsIURI* aURI, nsIURI* aOriginalURI,
                         Maybe<nsCOMPtr<nsIURI>> const& aResultPrincipalURI,
                         bool aKeepResultPrincipalURIIfSet, bool aLoadReplace,
                         ...
                         nsIPrincipal* aTriggeringPrincipal,
                         nsIPrincipal* aPrincipalToInherit, uint32_t aFlags,
                         const nsAString& aWindowTarget, const char* aTypeHint,
                         const nsAString& aFileName, nsIInputStream* aPostData,
                         int64_t aPostDataLength, nsIInputStream* aHeadersData,
                         uint32_t aLoadType, nsISHEntry* aSHEntry,
                         bool aFirstParty, const nsAString& aSrcdoc,
                         nsIDocShell* aSourceDocShell, nsIURI* aBaseURI,
  ...
  if (!targetDocShell && !aWindowTarget.IsEmpty()) {
    ...
    rv = NS_CheckContentLoadPolicy(contentType, aURI,
  ...
  if (!aWindowTarget.IsEmpty()) {
    ...
    if (!targetDocShell) {
      ...
      if (aFlags & INTERNAL_LOAD_FLAGS_NO_OPENER) {
        ...
        loadInfo->SetTriggeringPrincipal(aTriggeringPrincipal);
        loadInfo->SetInheritPrincipal(aFlags &
    ...
    if (NS_SUCCEEDED(rv) && targetDocShell) {
      rv = targetDocShell->InternalLoad(
          ...
          aTypeHint,
          VoidString(),  // No forced download
          aPostData, aPostDataLength, aHeadersData, aLoadType, aSHEntry,
          aFirstParty, aSrcdoc, aSourceDocShell, aBaseURI, aDocShell, aRequest);
  ...
  if (mFiredUnloadEvent) {
    if (IsOKToLoadURI(aURI)) {
      ...
      nsCOMPtr<nsIRunnable> ev = new InternalLoadEvent(
          ...
          aPostData, aPostDataLength, aHeadersData, aLoadType, aSHEntry,
          aFirstParty, aSrcdoc, aSourceDocShell, aBaseURI);
    ...
    if (doShortCircuitedLoad) {
      ...
      if (mOSHE) {
        mOSHE->GetTriggeringPrincipal(
            getter_AddRefs(newURITriggeringPrincipal));
  ...
  net::PredictorLearn(aURI, nullptr, nsINetworkPredictor::LEARN_LOAD_TOPLEVEL,
                      attrs);
  net::PredictorPredict(aURI, nullptr, nsINetworkPredictor::PREDICT_LOAD, attrs,
                        nullptr);
  ...
  rv = DoURILoad(
      ...
      aPostDataLength, aHeadersData, aFirstParty, aDocShell,
      getter_AddRefs(req), (aFlags & INTERNAL_LOAD_FLAGS_FIRST_LOAD) != 0,
...
nsresult nsDocShell::DoURILoad(
    ...
    const char* aTypeHint, const nsAString& aFileName,
    nsIInputStream* aPostData, int64_t aPostDataLength,
    nsIInputStream* aHeadersData, bool aFirstParty, nsIDocShell** aDocShell,
    nsIRequest** aRequest, bool aIsNewWindowTarget, bool aBypassClassifier,
    bool aForceAllowCookies, const nsAString& aSrcdoc, nsIURI* aBaseURI,
...
nsresult nsDocShell::DoChannelLoad(nsIChannel* aChannel,
  ...
  rv = AddClientChannelHelper(aChannel, Move(noReservedClient),
...
bool nsDocShell::OnNewURI(nsIURI* aURI, nsIChannel* aChannel,
  ...
  NS_PRECONDITION(!aChannel || !aTriggeringPrincipal,
...
nsresult nsDocShell::LoadHistoryEntry(nsISHEntry* aEntry, uint32_t aLoadType) {
  ...
  NS_ENSURE_SUCCESS(aEntry->GetOriginalURI(getter_AddRefs(originalURI)),
                    NS_ERROR_FAILURE);
  NS_ENSURE_SUCCESS(
      aEntry->GetResultPrincipalURI(getter_AddRefs(resultPrincipalURI)),
      NS_ERROR_FAILURE);
  NS_ENSURE_SUCCESS(aEntry->GetLoadReplace(&loadReplace), NS_ERROR_FAILURE);
  ...
  NS_ENSURE_SUCCESS(
      aEntry->GetTriggeringPrincipal(getter_AddRefs(triggeringPrincipal)),
      NS_ERROR_FAILURE);
  NS_ENSURE_SUCCESS(
      aEntry->GetPrincipalToInherit(getter_AddRefs(principalToInherit)),
      NS_ERROR_FAILURE);
...
nsresult nsDocShell::GetRootSessionHistory(nsISHistory** aReturn) {
  nsresult rv;
...
void nsDocShell::AddURIVisit(nsIURI* aURI, nsIURI* aReferrerURI,
  ...
  if (history) {
    ...
    (void)mGlobalHistory->AddURI(aURI, !!aChannelRedirectFlags, !IsFrame(),
                                 aReferrerURI);
  }
}
...
nsresult nsDocShell::EnsureTransferableHookData() {
  MOZ_ASSERT(!mIsBeingDestroyed);
...
NS_IMETHODIMP
nsDocShell::CutSelection(void) { return DoCommand("cmd_cut"); }

NS_IMETHODIMP
nsDocShell::CopySelection(void) { return DoCommand("cmd_copy"); }

NS_IMETHODIMP
nsDocShell::CopyLinkLocation(void) { return DoCommand("cmd_copyLink"); }
...
NS_IMETHODIMP
nsDocShell::SelectAll(void) { return DoCommand("cmd_selectAll"); }
...
//
NS_IMETHODIMP
nsDocShell::SelectNone(void) { return DoCommand("cmd_selectNone"); }
...
class OnLinkClickEvent : public Runnable {
 public:
  OnLinkClickEvent(nsDocShell* aHandler, nsIContent* aContent, nsIURI* aURI,
                   const char16_t* aTargetSpec, const nsAString& aFileName,
  ...
  NS_IMETHOD Run() override {
    ...
    if (mIsTrusted || jsapi.Init(mContent->OwnerDoc()->GetScopeObject())) {
      mHandler->OnLinkClickSync(mContent, mURI, mTargetSpec.get(), mFileName,
                                mPostDataStream, mPostDataStreamLength,
                                mHeadersDataStream, mNoOpenerImplied, nullptr,
                                nullptr, mTriggeringPrincipal);
    }
    return NS_OK;
  }
...
OnLinkClickEvent::OnLinkClickEvent(nsDocShell* aHandler, nsIContent* aContent,
                                   nsIURI* aURI, const char16_t* aTargetSpec,
...
nsDocShell::OnLinkClick(nsIContent* aContent, nsIURI* aURI,
                        ...
                        nsIInputStream* aPostDataStream,
                        int64_t aPostDataStreamLength,
                        nsIInputStream* aHeadersDataStream, bool aIsTrusted,
                        nsIPrincipal* aTriggeringPrincipal) {
  ...
  if (browserChrome3) {
    ...
    rv = browserChrome3->OnBeforeLinkTraversal(oldTarget, aURI, linkNode,
                                               mIsAppTab, target);
  ...
  nsCOMPtr<nsIRunnable> ev = new OnLinkClickEvent(
      this, aContent, aURI, target.get(), aFileName, aPostDataStream,
      aPostDataStreamLength, aHeadersDataStream, noOpenerImplied, aIsTrusted,
...
nsDocShell::OnLinkClickSync(
    nsIContent* aContent, nsIURI* aURI, const char16_t* aTargetSpec,
  ...
  nsresult rv =
      InternalLoad(clonedURI,  // New URI
...
nsDocShell::OnOverLink(nsIContent* aContent, nsIURI* aURI,
  ...
  mozilla::net::PredictorPredict(
      aURI, mCurrentURI, nsINetworkPredictor::PREDICT_LINK,
...
/* [infallible] */ NS_IMETHODIMP nsDocShell::SetFrameType(uint32_t aFrameType) {
  ...
  if (mJSRunToCompletionDepth == 0) {
    ...
    if (timelines && timelines->HasConsumer(this)) {
      timelines->AddMarkerForDocShell(
          this, Move(mozilla::MakeUnique<JavascriptTimelineMarker>(
