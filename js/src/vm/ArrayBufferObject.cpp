...
bool js::ToClampedIndex(JSContext* cx, HandleValue v, uint32_t length,
                        uint32_t* out) {
  int32_t result;
  if (!ToInt32(cx, v, &result)) return false;
  if (result < 0) {
    result += length;
...
void* js::MapBufferMemory(size_t mappedSize, size_t initialCommittedSize) {
  ...
  if (++liveBufferCount >= MaximumLiveMappedBuffers) {
    if (OnLargeAllocationFailure) OnLargeAllocationFailure();
    if (liveBufferCount >= MaximumLiveMappedBuffers) {
      liveBufferCount--;
      return nullptr;
    }
...
#ifdef XP_WIN
  if (!VirtualAlloc(dataEnd, delta, MEM_COMMIT, PAGE_READWRITE)) return false;
#else   // XP_WIN
  if (mprotect(dataEnd, delta, PROT_READ | PROT_WRITE)) return false;
#endif  // !XP_WIN
...
bool js::ExtendBufferMapping(void* dataPointer, size_t mappedSize,
  ...
  if (!VirtualAlloc(mappedEnd, delta, MEM_RESERVE, PAGE_NOACCESS)) return false;
  return true;
...
bool ArrayBufferObject::class_constructor(JSContext* cx, unsigned argc,
  ...
  // Step 1.
  if (!ThrowIfNotConstructing(cx, args, "ArrayBuffer")) return false;

  // Step 2.
  uint64_t byteLength;
  if (!ToIndex(cx, args.get(0), &byteLength)) return false;
...
class js::WasmArrayRawBuffer {
 ...
 public:
  ...
  size_t allocatedBytes() const { return mappedSize_ + gc::SystemPageSize(); }
  ...
  MOZ_MUST_USE bool growToSizeInPlace(uint32_t oldSize, uint32_t newSize) {
    ...
    if (delta && !CommitBufferMemory(dataEnd, delta)) return false;
  ...
  bool extendMappedSize(uint32_t maxSize) {
    ...
    if (mappedSize_ == newMappedSize) return true;
  ...
  void tryGrowMaxSizeInPlace(uint32_t deltaMaxSize) {
    ...
    if (!extendMappedSize(newMaxSize.value())) return;
...
/* static */ bool ArrayBufferObject::prepareForAsmJS(
    JSContext* cx, Handle<ArrayBufferObject*> buffer, bool needGuard) {
#ifdef WASM_HUGE_MEMORY
  ...
  if (buffer->forInlineTypedObject()) return false;

  if (needGuard) {
    if (buffer->isWasm() && buffer->isPreparedForAsmJS()) return true;
    ...
    if (buffer->isWasm() || buffer->isPreparedForAsmJS()) return false;

    uint32_t length = buffer->byteLength();
    WasmArrayRawBuffer* wasmBuf =
        WasmArrayRawBuffer::Allocate(length, Some(length));
    if (!wasmBuf) {
      ReportOutOfMemory(cx);
      return false;
    }
...
ArrayBufferObject::BufferContents ArrayBufferObject::createMappedContents(
  ...
  void* data =
      AllocateMappedContent(fd, offset, length, ARRAY_BUFFER_ALIGNMENT);
...
ArrayBufferObject::RefcountInfo* ArrayBufferObject::refcountInfo() const {
...
size_t ArrayBufferObject::wasmMappedSize() const {
  if (isWasm()) return contents().wasmBuffer()->mappedSize();
  return byteLength();
}
...
/* static */ bool ArrayBufferObject::wasmGrowToSizeInPlace(
  ...
  if (newSize > ArrayBufferObject::MaxBufferByteLength) return false;
...
uint32_t ArrayBufferObject::wasmBoundsCheckLimit() const {
  if (isWasm()) return contents().wasmBuffer()->boundsCheckLimit();
  return byteLength();
}
...
ArrayBufferObject* ArrayBufferObject::create(
  ...
  if (contents) {
    if (ownsState == OwnsData) {
      if (contents.kind() == EXTERNAL) {
        ...
        size_t refcountInfoSlots =
            JS_HOWMANY(sizeof(RefcountInfo), sizeof(Value));
        MOZ_ASSERT(
            reservedSlots + refcountInfoSlots <= NativeObject::MAX_FIXED_SLOTS,
  ...
  if (!obj) {
    if (allocated) js_free(contents.data());
    return nullptr;
  }
...
ArrayBufferObject* ArrayBufferObject::createEmpty(JSContext* cx) {
  AutoSetNewObjectMetadata metadata(cx);
  ArrayBufferObject* obj =
      NewObjectWithClassProto<ArrayBufferObject>(cx, nullptr);
...
ArrayBufferObject::externalizeContents(JSContext* cx,
  ...
  BufferContents newContents =
      AllocateArrayBufferContents(cx, buffer->byteLength());
  if (!newContents) return BufferContents::createPlain(nullptr);
...
/* static */ ArrayBufferObject::BufferContents ArrayBufferObject::stealContents(
  ...
  BufferContents contentsCopy =
      AllocateArrayBufferContents(cx, buffer->byteLength());
  if (!contentsCopy) return BufferContents::createPlain(nullptr);
...
/* static */ void ArrayBufferObject::finalize(FreeOp* fop, JSObject* obj) {
  ...
  if (buffer.ownsData()) buffer.releaseData(fop);
}
...
/* static */ void ArrayBufferObject::trace(JSTracer* trc, JSObject* obj) {
  ...
  if (!buf.forInlineTypedObject()) return;
...
ArrayBufferViewObject* ArrayBufferObject::firstView() {
  return getSlot(FIRST_VIEW_SLOT).isObject()
             ? static_cast<ArrayBufferViewObject*>(
                   &getSlot(FIRST_VIEW_SLOT).toObject())
             : nullptr;
}
...
bool ArrayBufferObject::addView(JSContext* cx, JSObject* viewArg) {
  ...
  MOZ_ASSERT(viewArg->is<ArrayBufferViewObject>() ||
             viewArg->is<TypedObject>());
...
bool InnerViewTable::addView(JSContext* cx, ArrayBufferObject* buffer,
  ...
  if (addToNursery && !nurseryKeys.append(buffer)) nurseryKeysValid = false;
...
InnerViewTable::ViewVector* InnerViewTable::maybeViewsUnbarriered(
  ...
  if (!map.initialized()) return nullptr;
...
/* static */ bool InnerViewTable::sweepEntry(JSObject** pkey,
                                             ViewVector& views) {
  if (IsAboutToBeFinalizedUnbarriered(pkey)) return true;
...
void InnerViewTable::sweepAfterMinorGC() {
  ...
  if (nurseryKeysValid) {
    for (size_t i = 0; i < nurseryKeys.length(); i++) {
      ...
      if (sweepEntry(&p->mutableKey(), p->value())) map.remove(buffer);
    }
    nurseryKeys.clear();
  } else {
...
size_t InnerViewTable::sizeOfExcludingThis(mozilla::MallocSizeOf mallocSizeOf) {
  ...
  return vectorSize + map.sizeOfExcludingThis(mallocSizeOf) +
  ...
  if (bufSlot.isObject()) {
    if (IsArrayBuffer(&bufSlot.toObject())) {
      ...
      uint32_t offset = uint32_t(
          obj->getFixedSlot(TypedArrayObject::BYTEOFFSET_SLOT).toInt32());
      ...
      if (buf.forInlineTypedObject()) {
        ...
        void* dstData =
            view->as<InlineTypedObject>().inlineTypedMemForGC() + offset;
        ...
        if (trc->isTenuringTracer()) {
          ...
          nursery.maybeSetForwardingPointer(trc, srcData, dstData,
                                            /* direct = */ false);
        }
      } else {
...
void ArrayBufferViewObject::notifyBufferDetached(JSContext* cx, void* newData) {
  if (is<DataViewObject>()) {
    if (as<DataViewObject>().isSharedMemory()) return;
    as<DataViewObject>().notifyBufferDetached(newData);
  } else if (is<TypedArrayObject>()) {
    if (as<TypedArrayObject>().isSharedMemory()) return;
...
#ifdef DEBUG
bool ArrayBufferViewObject::isSharedMemory() {
  if (is<TypedArrayObject>()) return as<TypedArrayObject>().isSharedMemory();
  return false;
}
#endif
...
/* static */ ArrayBufferObjectMaybeShared* ArrayBufferViewObject::bufferObject(
  ...
  if (thisObject->is<TypedArrayObject>()) {
    ...
    if (!TypedArrayObject::ensureHasBuffer(cx, typedArray)) return nullptr;
...
JS_FRIEND_API uint8_t* JS_GetArrayBufferData(JSObject* obj,
  ...
  obj = CheckedUnwrap(obj);
  if (!obj) return nullptr;
  if (!IsArrayBuffer(obj)) return nullptr;
...
JS_PUBLIC_API JSObject* JS_NewExternalArrayBuffer(
    JSContext* cx, size_t nbytes, void* data, JS::BufferContentsRefFunc ref,
    JS::BufferContentsRefFunc unref, void* refUserData) {
  AssertHeapIsIdle();
  CHECK_REQUEST(cx);
  ...
  ArrayBufferObject::BufferContents contents =
      ArrayBufferObject::BufferContents::createExternal(data, ref, unref,
                                                        refUserData);
  return ArrayBufferObject::create(cx, nbytes, contents,
                                   ArrayBufferObject::OwnsData,
                                   /* proto = */ nullptr, TenuredObject);
...
JS_FRIEND_API void* JS_GetArrayBufferViewData(JSObject* obj,
  ...
  if (obj->is<DataViewObject>()) {
    ...
    *isSharedMemory = dv.isSharedMemory();
    return dv.dataPointerEither().unwrap(
...
JS_FRIEND_API JSObject* JS_GetArrayBufferViewBuffer(JSContext* cx,
  ...
  Rooted<ArrayBufferViewObject*> viewObject(
      cx, static_cast<ArrayBufferViewObject*>(obj));
  ArrayBufferObjectMaybeShared* buffer =
      ArrayBufferViewObject::bufferObject(cx, viewObject);
...
JS_FRIEND_API JSObject* JS_GetObjectAsArrayBufferView(JSObject* obj,
  ...
  if (!(obj->is<ArrayBufferViewObject>())) return nullptr;
