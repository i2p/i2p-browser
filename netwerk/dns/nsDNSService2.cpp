...
class nsDNSAsyncRequest final : public nsResolveHostCallback,
 ...
 public:
  ...
  nsDNSAsyncRequest(nsHostResolver *res, const nsACString &host,
                    ...
                    uint16_t flags, uint16_t af, const nsACString &netInterface)
      : mResolver(res),
        mHost(host),
...
nsDNSService::Init() {
  ...
  nsresult rv = nsHostResolver::Create(maxCacheEntries, defaultCacheLifetime,
                                       defaultGracePeriod, getter_AddRefs(res));
  if (NS_SUCCEEDED(rv)) {
    ...
    // Disable prefetching either by explicit preference or if a manual proxy is
    // configured
    mDisablePrefetch =
        disablePrefetch ||
...
nsDNSService::AsyncResolve(const nsACString &aHostname, uint32_t flags,
  ...
  return AsyncResolveExtendedNative(aHostname, flags, EmptyCString(), listener,
...
NS_IMETHODIMP
nsDNSService::AsyncResolveExtended(const nsACString &aHostname, uint32_t flags,
...
NS_IMETHODIMP
nsDNSService::AsyncResolveExtendedNative(
    const nsACString &aHostname, uint32_t flags,
    const nsACString &aNetworkInterface, nsIDNSListener *aListener,
  ...
  RefPtr<nsDNSAsyncRequest> req = new nsDNSAsyncRequest(
      res, hostname, aOriginAttributes, listener, flags, af, aNetworkInterface);
...
nsDNSService::CancelAsyncResolveNative(
  ...
  return CancelAsyncResolveExtendedNative(
      aHostname, aFlags, EmptyCString(), aListener, aReason, aOriginAttributes);
}
...
NS_IMETHODIMP
nsDNSService::CancelAsyncResolveExtendedNative(
    const nsACString &aHostname, uint32_t aFlags,
    const nsACString &aNetworkInterface, nsIDNSListener *aListener,
    nsresult aReason, const OriginAttributes &aOriginAttributes) {
  // grab reference to global host resolver and IDN service.  beware
  // simultaneous shutdown!!
  ...
  res->CancelAsyncRequest(hostname.get(), aOriginAttributes, aFlags, af,
                          nsPromiseFlatCString(aNetworkInterface).get(),
                          aListener, aReason);
  return NS_OK;
}
...
nsresult nsDNSService::ResolveInternal(
  ...
  rv = res->ResolveHost(hostname.get(), aOriginAttributes, flags, af, "",
                        syncReq);
  if (NS_SUCCEEDED(rv)) {
    // wait for result
    while (!syncReq->mDone) {
...
nsDNSService::Observe(nsISupports *subject, const char *topic,
  ...
  //
  // Shutdown and this function are both only called on the UI thread, so we
  // don't have to worry about mResolver being cleared out from under us.
  //
