...
#ifdef DEBUG
#define NS_DEFINE_CLASSINFO_DATA_DEBUG(_class) eDOMClassInfo_##_class##_id,
#else
#define NS_DEFINE_CLASSINFO_DATA_DEBUG(_class)  // nothing
#endif

#define NS_DEFINE_CLASSINFO_DATA_HELPER(_class, _helper, _flags, _chromeOnly, \
                                        _allowXBL)                            \
  {nullptr,                                                                   \
...
static nsDOMClassInfoData sClassInfoData[] = {
    ...
    NS_DEFINE_CLASSINFO_DATA(
        DOMPrototype, nsDOMConstructorSH,
        DOM_BASE_SCRIPTABLE_FLAGS | XPC_SCRIPTABLE_WANT_PRECREATE |
            XPC_SCRIPTABLE_WANT_RESOLVE | XPC_SCRIPTABLE_WANT_HASINSTANCE |
            XPC_SCRIPTABLE_DONT_ENUM_QUERY_INTERFACE)
        NS_DEFINE_CLASSINFO_DATA(
            DOMConstructor, nsDOMConstructorSH,
            DOM_BASE_SCRIPTABLE_FLAGS | XPC_SCRIPTABLE_WANT_PRECREATE |
                XPC_SCRIPTABLE_WANT_RESOLVE | XPC_SCRIPTABLE_WANT_HASINSTANCE |
    ...
    NS_DEFINE_CHROME_ONLY_CLASSINFO_DATA(
        ContentFrameMessageManager, nsMessageManagerSH<nsEventTargetSH>,
        DOM_DEFAULT_SCRIPTABLE_FLAGS | XPC_SCRIPTABLE_WANT_ENUMERATE |
            XPC_SCRIPTABLE_IS_GLOBAL_OBJECT)
        NS_DEFINE_CHROME_ONLY_CLASSINFO_DATA(
            ContentProcessMessageManager, nsMessageManagerSH<nsDOMGenericSH>,
            DOM_DEFAULT_SCRIPTABLE_FLAGS | XPC_SCRIPTABLE_WANT_ENUMERATE |
                XPC_SCRIPTABLE_IS_GLOBAL_OBJECT)
            NS_DEFINE_CHROME_ONLY_CLASSINFO_DATA(ChromeMessageBroadcaster,
                                                 nsDOMGenericSH,
                                                 DOM_DEFAULT_SCRIPTABLE_FLAGS)
                NS_DEFINE_CHROME_ONLY_CLASSINFO_DATA(
                    ChromeMessageSender, nsDOMGenericSH,
                    DOM_DEFAULT_SCRIPTABLE_FLAGS)};
 ...
 */
static void FindObjectClass(JSContext *cx, JSObject *aGlobalObject) {
  NS_ASSERTION(!sObjectClass, "Double set of sObjectClass");
...
static inline nsresult SetParentToWindow(nsGlobalWindowInner *win,
                                         JSObject **parent) {
  MOZ_ASSERT(win);
...
nsresult nsDOMClassInfo::DefineStaticJSVals() {
  AutoJSAPI jsapi;
...
// static
bool nsDOMClassInfo::ObjectIsNativeWrapper(JSContext *cx, JSObject *obj) {
...
NS_INTERFACE_MAP_BEGIN(nsDOMClassInfo)
  if (aIID.Equals(NS_GET_IID(nsXPCClassInfo)))
    foundInterface =
        static_cast<nsIClassInfo *>(static_cast<nsXPCClassInfo *>(this));
  else
...
static const JSClass sDOMConstructorProtoClass = {"DOM Constructor.prototype",
                                                  0};
...
// static
nsresult nsDOMClassInfo::RegisterClassProtos(int32_t aClassInfoID) {
...
nsresult nsDOMClassInfo::Init() {
  ...
  nsCOMPtr<nsIXPCFunctionThisTranslator> elt =
  ...
  for (size_t i = 0; i < eDOMClassInfoIDCount; i++) {
    if (!sClassInfoData[i].mConstructorFptr ||
      ...
      MOZ_CRASH(
          "Class info data out of sync, you forgot to update "
  ...
  for (size_t i = 0; i < eDOMClassInfoIDCount; i++) {
    if (!sClassInfoData[i].mInterfaces) {
      MOZ_CRASH(
          "Class info data without an interface list! Fix this, "
          "mozilla will not work without this fixed!");
    }
  }
#endif
...
// virtual
uint32_t nsDOMClassInfo::GetScriptableFlags() {
  return mData->mScriptableFlags;
}

// virtual
const js::Class *nsDOMClassInfo::GetClass() { return &mData->mClass; }

// virtual
const JSClass *nsDOMClassInfo::GetJSClass() { return Jsvalify(&mData->mClass); }
...
nsDOMClassInfo::Resolve(nsIXPConnectWrappedNative *wrapper, JSContext *cx,
  ...
  if (desc.object() && !desc.hasGetterOrSetter() && desc.value().isObject()) {
    ...
    if (!::JS_DefinePropertyById(cx, obj, id, desc.value(), JSPROP_ENUMERATE)) {
      return NS_ERROR_UNEXPECTED;
    }
...
static nsresult ResolvePrototype(
    nsIXPConnect *aXPConnect, nsGlobalWindowInner *aWin, JSContext *cx,
    JS::Handle<JSObject *> obj, const char16_t *name,
    const nsDOMClassInfoData *ci_data, const nsGlobalNameStruct *name_struct,
    nsScriptNameSpaceManager *nameSpaceManager, JSObject *dot_prototype,
...
nsDOMClassInfo::PostCreatePrototype(JSContext *cx, JSObject *aProto) {
  ...
  if (mData->mHasClassInterface && mData->mProtoChainInterface &&
    ...
    if (iim) {
      nsCOMPtr<nsIInterfaceInfo> if_info;
      iim->GetInfoForIID(mData->mProtoChainInterface, getter_AddRefs(if_info));

      if (if_info) {
        nsCString name;
        if_info->GetName(getter_Copies(name));
        NS_ASSERTION(
            nsCRT::strcmp(CutPrefix(name.get()), mData->mClass.name) == 0,
  ...
  if (!::JS_AlreadyHasOwnUCProperty(
          cx, global, reinterpret_cast<const char16_t *>(mData->mNameUTF16),
          NS_strlen(mData->mNameUTF16), &contentDefinedProperty)) {
    return NS_ERROR_FAILURE;
  }
  ...
  JS::Rooted<JS::PropertyDescriptor> desc(cx);
  nsresult rv =
      ResolvePrototype(sXPConnect, win, cx, global, mData->mNameUTF16, mData,
                       nullptr, nameSpaceManager, proto, &desc);
...
// static
nsIClassInfo *NS_GetDOMClassInfoInstance(nsDOMClassInfoID aID) {
...
class nsDOMConstructor final : public nsIDOMDOMConstructor {
 protected:
  nsDOMConstructor(const char16_t *aName, bool aIsConstructable,
 ...
 public:
  ...
  nsresult HasInstance(nsIXPConnectWrappedNative *wrapper, JSContext *cx,
                       JS::Handle<JSObject *> obj, const JS::Value &val,
...
// static
nsresult nsDOMConstructor::Create(const char16_t *aName,
                                  const nsGlobalNameStruct *aNameStruct,
                                  nsPIDOMWindowInner *aOwner,
                                  nsDOMConstructor **aResult) {
  *aResult = nullptr;
...
NS_INTERFACE_MAP_BEGIN(nsDOMConstructor)
  ...
  if (aIID.Equals(NS_GET_IID(nsIClassInfo))) {
#ifdef DEBUG
    {
      ...
      NS_ASSERTION(
          !name_struct || mConstructable == IsConstructable(name_struct),
          "Can't change constructability dynamically!");
    }
#endif
    foundInterface = NS_GetDOMClassInfoInstance(
        mConstructable ? eDOMClassInfo_DOMConstructor_id
...
nsresult nsDOMConstructor::PreCreate(JSContext *cx, JSObject *globalObj,
                                     JSObject **parentObj) {
...

{
  ...
  *bp = false;
  ...
  // This might not be the right object, if there are wrappers. Unwrap if we
  // can.
  JSObject *wrapped_obj =
      js::CheckedUnwrap(dom_obj, /* stopAtWindowProxy = */ false);
  if (wrapped_obj) dom_obj = wrapped_obj;
  ...
  const nsGlobalNameStruct *name_struct;
  nsresult rv =
      GetNameStruct(NS_ConvertASCIItoUTF16(dom_class->name), &name_struct);
  if (NS_FAILED(rv)) {
    return rv;
  }
  ...
  if (!name_struct) {
    ...
    if (!desc.object() || desc.hasGetterOrSetter() ||
        !desc.value().isObject()) {
      return NS_OK;
    }
  ...
  } else if (class_name_struct->mType ==
...
nsresult nsDOMConstructor::ResolveInterfaceConstants(
    JSContext *cx, JS::Handle<JSObject *> obj) {
  const nsGlobalNameStruct *class_name_struct = GetNameStruct();
  if (!class_name_struct) return NS_ERROR_UNEXPECTED;
...
static nsresult GetXPCProto(nsIXPConnect *aXPConnect, JSContext *cx,
                            nsGlobalWindowInner *aWin,
                            const nsGlobalNameStruct *aNameStruct,
  ...
  nsresult rv = aXPConnect->GetWrappedNativePrototype(
      cx, aWin->GetGlobalJSObject(), ci, aProto.address());
  NS_ENSURE_SUCCESS(rv, rv);
...
static nsresult ResolvePrototype(
    ...
    JS::Handle<JSObject *> obj, const char16_t *name,
    const nsDOMClassInfoData *ci_data, const nsGlobalNameStruct *name_struct,
    nsScriptNameSpaceManager *nameSpaceManager, JSObject *aDot_prototype,
    JS::MutableHandle<JS::PropertyDescriptor> ctorDesc) {
  ...
  rv = nsContentUtils::WrapNative(cx, constructor,
                                  &NS_GET_IID(nsIDOMDOMConstructor), &v, false);
  NS_ENSURE_SUCCESS(rv, rv);
...

  {
    ...
    if (class_parent_name) {
      ...
      JS::Rooted<JS::PropertyDescriptor> desc(cx);
      if (!JS_GetPropertyDescriptor(cx, winobj, CutPrefix(class_parent_name),
                                    &desc)) {
        return NS_ERROR_UNEXPECTED;
      }
    ...
    if (dot_prototype) {
      ...
      if (proto &&
          (!xpc_proto_proto || JS_GetClass(xpc_proto_proto) == sObjectClass)) {
      ...
      dot_prototype =
          ::JS_NewObjectWithUniqueType(cx, &sDOMConstructorProtoClass, proto);
...
static bool OldBindingConstructorEnabled(const nsGlobalNameStruct *aStruct,
                                         nsGlobalWindowInner *aWin,
                                         JSContext *cx) {
...
// static
bool nsWindowSH::NameStructEnabled(JSContext *aCx, nsGlobalWindowInner *aWin,
...
// static
nsresult nsWindowSH::GlobalResolve(
    nsGlobalWindowInner *aWin, JSContext *cx, JS::Handle<JSObject *> obj,
  ...
  if ((id == XPCJSRuntime::Get()->GetStringID(XPCJSContext::IDX_CONTROLLERS) ||
       id == XPCJSRuntime::Get()->GetStringID(
    ...
    FillPropertyDescriptor(desc, obj, JS::ObjectValue(*shim),
                           /* readOnly = */ false);
    return NS_OK;
  }
#endif
  ...
  if (name_struct->mType == nsGlobalNameStruct::eTypeClassProto) {
    ...
    // one.
    return ResolvePrototype(nsDOMClassInfo::sXPConnect, aWin, cx, obj,
                            class_name, nullptr, name_struct, nameSpaceManager,
                            nullptr, desc);
  }
  ...
  if (name_struct->mType == nsGlobalNameStruct::eTypeExternalConstructor) {
    ...
    js::AssertSameCompartment(cx, obj);
    rv = nsContentUtils::WrapNative(
        cx, constructor, &NS_GET_IID(nsIDOMDOMConstructor), &val, true);
    NS_ENSURE_SUCCESS(rv, rv);
  ...
  if (name_struct->mType == nsGlobalNameStruct::eTypeProperty) {
    if (!OldBindingConstructorEnabled(name_struct, aWin, cx)) return NS_OK;
...
void nsEventTargetSH::PreserveWrapper(nsISupports *aNative) {
...
nsDOMConstructorSH::Resolve(nsIXPConnectWrappedNative *wrapper, JSContext *cx,
  ...
  JS::Rooted<JSObject *> nativePropsObj(
      cx, xpc::XrayUtils::GetNativePropertiesObject(cx, obj));
...
nsDOMConstructorSH::Call(nsIXPConnectWrappedNative *wrapper, JSContext *cx,
                         JSObject *aObj, const JS::CallArgs &args,
                         bool *_retval) {
  JS::Rooted<JSObject *> obj(cx, aObj);
  MOZ_ASSERT(obj);
...
nsDOMConstructorSH::HasInstance(nsIXPConnectWrappedNative *wrapper,
                                JSContext *cx, JSObject *aObj,
                                JS::Handle<JS::Value> val, bool *bp,
                                bool *_retval) {
  JS::Rooted<JSObject *> obj(cx, aObj);
  nsDOMConstructor *wrapped =
      static_cast<nsDOMConstructor *>(wrapper->Native());
...
NS_IMETHODIMP nsMessageManagerSH<Super>::Resolve(
    nsIXPConnectWrappedNative *wrapper, JSContext *cx, JSObject *obj_, jsid id_,
...
template <typename Super>
NS_IMETHODIMP nsMessageManagerSH<Super>::Enumerate(
    nsIXPConnectWrappedNative *wrapper, JSContext *cx, JSObject *obj_,
    bool *_retval) {
  JS::Rooted<JSObject *> obj(cx, obj_);
