...
NS_INTERFACE_MAP_BEGIN_CYCLE_COLLECTION(nsGlobalWindowOuter)
  ...
  } else if (aIID.Equals(NS_GET_IID(mozIDOMWindowProxy))) {
    foundInterface = static_cast<mozIDOMWindowProxy*>(this);
  } else if (aIID.Equals(NS_GET_IID(nsIDOMChromeWindow)) && IsChromeWindow()) {
...
void nsGlobalWindowOuter::TraceGlobalJSObject(JSTracer* aTrc) {
...
static JS::CompartmentCreationOptions& SelectZoneGroup(
    nsGlobalWindowInner* aNewInner, JS::CompartmentCreationOptions& aOptions) {
...
nsresult nsGlobalWindowOuter::SetNewDocument(nsIDocument* aDocument,
  ...
  if (reUseInnerWindow) {
    ...
    // compartment to match the new document. NB: We don't just call
    // currentInner->RefreshCompartmentPrincipals() here because we haven't yet
    // set its mDoc to aDocument.
    ...
    if (aDocument != oldDoc) {
      JS_SetCompartmentPrincipals(
          compartment, nsJSPrincipals::get(aDocument->NodePrincipal()));
    ...
    if (currentInner && currentInner->GetWrapperPreserveColor()) {
      if (oldDoc == aDocument) {
        ...
        if (currentInner->mPerformance) {
          newInnerWindow->mPerformance = Performance::CreateForMainThread(
              newInnerWindow->AsInner(), aDocument->NodePrincipal(),
    ...
    if (!GetWrapperPreserveColor()) {
      ...
      MOZ_ASSERT(js::GetGlobalForObjectCrossCompartment(outerObject) ==
                 newInnerGlobal);
  ...
  if (GetDocGroup() && !nsContentUtils::IsSystemPrincipal(principal) &&
    ...
    js::SetCompartmentValidAccessPtr(
...
void nsGlobalWindowOuter::DispatchDOMWindowCreated() {
  ...
  nsContentUtils::DispatchChromeEvent(
      mDoc, mDoc, NS_LITERAL_STRING("DOMWindowCreated"), true /* bubbles */,
...
EventTarget* nsGlobalWindowOuter::GetTargetForDOMEvent() {
...
nsresult nsGlobalWindowOuter::WillHandleEvent(EventChainPostVisitor& aVisitor) {
  return NS_OK;
}

nsresult nsGlobalWindowOuter::GetEventTargetParent(
...
nsIDOMScreen* nsGlobalWindowOuter::GetScreen() {
...
nsDOMWindowList* nsGlobalWindowOuter::GetWindowList() {
...
already_AddRefed<nsIDOMWindowCollection> nsGlobalWindowOuter::GetFrames() {
...
nsIControllers* nsGlobalWindowOuter::GetControllersOuter(ErrorResult& aError) {
  if (!mControllers) {
    ...
    nsCOMPtr<nsIController> controller =
        do_CreateInstance(NS_WINDOWCONTROLLER_CONTRACTID, &rv);
    if (NS_FAILED(rv)) {
      aError.Throw(rv);
      return nullptr;
    }
...
bool nsGlobalWindowOuter::DispatchCustomEvent(const nsAString& aEventName) {
  ...
  nsContentUtils::DispatchTrustedEvent(mDoc, ToSupports(this), aEventName, true,
                                       true, &defaultActionEnabled);
...
bool nsGlobalWindowOuter::DispatchResizeEvent(const CSSIntSize& aSize) {
  ...
  customEvent->InitCustomEvent(cx, NS_LITERAL_STRING("DOMWindowResize"),
                               /* aCanBubble = */ true,
                               /* aCancelable = */ true, detailValue, res);
  if (res.Failed()) {
    return false;
  }
...
void nsGlobalWindowOuter::BackOuter(ErrorResult& aError) {
...
void nsGlobalWindowOuter::ForwardOuter(ErrorResult& aError) {
  nsCOMPtr<nsIWebNavigation> webNav(do_QueryInterface(mDocShell));
  if (!webNav) {
    aError.Throw(NS_ERROR_FAILURE);
    return;
...
void nsGlobalWindowOuter::HomeOuter(nsIPrincipal& aSubjectPrincipal,
                                    ErrorResult& aError) {
  if (!mDocShell) {
    return;
  }
  ...
  aError = webNav->LoadURI(homeURL.get(), nsIWebNavigation::LOAD_FLAGS_NONE,
                           nullptr, nullptr, nullptr, &aSubjectPrincipal);
}
...
static bool IsPopupBlocked(nsIDocument* aDoc) {
...
bool nsGlobalWindowOuter::PopupWhitelisted() {
  if (!IsPopupBlocked(mDoc)) return true;
...
nsresult nsGlobalWindowOuter::Open(const nsAString& aUrl,
  ...
  return OpenInternal(aUrl, aName, aOptions,
                      ...
                      aLoadInfo, aForceNoOpener, _retval);
}
...
nsGlobalWindowInner* nsGlobalWindowOuter::CallerInnerWindow() {
 ...
 */
already_AddRefed<nsIDOMElement> nsGlobalWindowOuter::GetFrameElement() {
...
already_AddRefed<nsISelection> nsGlobalWindowOuter::GetSelection() {
...
void nsGlobalWindowOuter::AddEventListener(
...
nsGlobalWindowOuter::AddSystemEventListener(const nsAString& aType,
  ...
  if (mInnerWindow && !nsContentUtils::LegacyIsCallerNativeCode() &&
...
nsIScriptContext* nsGlobalWindowOuter::GetContextForEventHandlers(
    nsresult* aRv) {
...
void nsGlobalWindowOuter::SetFocusedNode(nsIContent* aNode,
...
void nsGlobalWindowOuter::SetKeyboardIndicators(
  ...
  if (mInnerWindow && nsGlobalWindowInner::Cast(mInnerWindow)->mHasFocus &&
    ...
    if (newShouldShowFocusRing) {
      mInnerWindow->mFocusedNode->AsElement()->AddStates(
          NS_EVENT_STATE_FOCUSRING);
    } else {
      mInnerWindow->mFocusedNode->AsElement()->RemoveStates(
          NS_EVENT_STATE_FOCUSRING);
    }
  }
}
...
nsresult nsGlobalWindowOuter::OpenInternal(
  ...
  *aReturn = nullptr;
  ...
  const bool checkForPopup =
      ...
      !WindowExists(aName, forceNoOpener, !aCalledNoScript);
...

  {
    ...
    if (!aCalledNoScript) {
      ...
      rv = pwwatch->OpenWindow2(
          ...
          isPopupSpamWindow, forceNoOpener, aLoadInfo,
...
bool nsGlobalWindowOuter::IsPrivateBrowsing() {
...
void nsGlobalWindowOuter::EventListenerAdded(nsAtom* aType) {}

void nsGlobalWindowOuter::EventListenerRemoved(nsAtom* aType) {}
...
bool nsGlobalWindowOuter::UpdateVRDisplays(
...
nsGlobalWindowOuter::GetGroupMessageManager(const nsAString& aGroup,
  ...
  MOZ_RELEASE_ASSERT(IsChromeWindow());
  FORWARD_TO_INNER(GetGroupMessageManager, (aGroup, aManager),
                   NS_ERROR_UNEXPECTED);
}
...
nsIURI* nsPIDOMWindowOuter::GetDocBaseURI() const {
...
nsPIDOMWindowOuter::nsPIDOMWindowOuter()
    : mFrameElement(nullptr),
      ...
      mIsActive(false),
      mIsBackground(false),
      mMediaSuspend(Preferences::GetBool(
