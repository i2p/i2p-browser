...
static const char* kPreloadPermissions[] = {
    ...
    "xslt", "beacon", "fetch", "image", "manifest",
...
NS_IMETHODIMP nsPermissionManager::GetEnumerator(nsISimpleEnumerator** aEnum) {
  if (XRE_IsContentProcess()) {
    NS_WARNING(
        "nsPermissionManager's enumerator is not available in the "
...
nsresult nsPermissionManager::RemoveAllModifiedSince(
  ...
  for (int32_t i = 0; i < array.Count(); ++i) {
    ...
    AddInternal(principal, type, nsIPermissionManager::UNKNOWN_ACTION, 0,
                nsIPermissionManager::EXPIRE_NEVER, 0, 0,
