...
const TOR_BROWSER_PAGE_ACTIONS_ALLOWED = new Set([
  // Add page actions used by I2P Browser's new user/feature onboarding here.
]);
    ...
    [
      "accountStatus",
      {
        query: aDocument => {
          ...
          return aDocument.getAnonymousElementByAttribute(
            statusButton,
            "class",
            "toolbarbutton-icon"
          );
    ...
    [
      "selectedTabIcon",
      {
        query: aDocument => {
          ...
          let element = aDocument.getAnonymousElementByAttribute(
            selectedtab,
            "anonid",
            "tab-icon-image"
          );
