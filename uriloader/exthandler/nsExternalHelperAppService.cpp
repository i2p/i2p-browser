...
class nsExternalLoadURIHandler final : public nsIHelperAppWarningLauncher {
 public:
  ...
  nsExternalLoadURIHandler(nsIInterfaceRequestor* aWindowContext, nsIURI* aURI,
...
nsExternalLoadURIHandler::nsExternalLoadURIHandler(
    nsIInterfaceRequestor* aWindowContext, nsIURI* aURI,
    nsIHandlerInfo* aHandlerInfo)
    : mWindowContext(aWindowContext), mURI(aURI), mHandlerInfo(aHandlerInfo) {
  nsresult rv = NS_OK;
...
NS_IMETHODIMP nsExternalHelperAppService::GetFromTypeAndExtension(
    ...
    nsIMIMEInfo** _retval) {
  NS_PRECONDITION(!aMIMEType.IsEmpty() || !aFileExt.IsEmpty(),
...
nsExternalHelperAppService::GetTypeFromExtension(const nsACString& aFileExt,
  ...
  if (catMan) {
    ...
    nsresult rv = catMan->GetCategoryEntry(
        "ext-to-type-mapping", lowercaseFileExt.get(), getter_Copies(type));
    if (NS_SUCCEEDED(rv)) {
      aContentType = type;
      return NS_OK;
    }
